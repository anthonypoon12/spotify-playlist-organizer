...
- **The API remembers adds, but not removes.** Playlist items and saved tracks carry `added_at` and `added_by`, so what was added — and roughly when, and by whom — is partially reconstructable after the fact. A removal leaves nothing behind: once a track is gone from a playlist or from Liked Songs, no endpoint can report that it was ever there. Any record of what this app has removed therefore has to be written locally at the moment the write happens, or it does not exist at all.
...
- **The outcome is three-valued** — confirmed, ambiguous-then-retried, or failed. This is forced by v1's own commit runner. An ambiguous write is normally settled by a single verification request, which lands it in one of the two definite states; but when the verification request itself fails, the runner falls back to retrying the write and accepts a duplicate as the worst case. That path ends with a write believed to have landed rather than known to have landed, which is a genuinely different state from either success or failure. The history is exactly where that difference gets consulted, so recording it as a success would hollow out the feature.
...
- **Undo of a committed change** is not planned. v8 records what the app wrote; it does not reverse it. Reversal fights decisions already made: removal by URI deletes every copy of a track, so an undo cannot know how many to restore; re-adding a track appends it rather than restoring its position, and rewrites its `added_at`; and the library keeps drifting after a commit, so an entry gets less safely reversible the longer it sits. The Spotify client remains the place a committed mistake gets fixed.
- **Cross-device history** is ruled out by the no-backend decision and by the storage-partition constraint in *Constraints and assumptions* — the partition an installed home-screen app is given is not shared with a Safari tab at the same origin, let alone with another device, so a history written in one is invisible to the other. v8's record format leaves room for a manual export and import to bridge this later, but neither is built.

...
Authentication uses the Authorization Code flow with PKCE, which means there is no backend and no client secret anywhere in the codebase. The app talks to the Spotify Web API directly from the browser. The build output is static files, deployable to any static host.
...
**Done when.** A real triage session moves real songs between real playlists end to end, and a deliberately failed mid-commit leaves every song either in its original playlist or in its new one — never in neither.
